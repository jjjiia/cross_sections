...
def getCentroids():
    with open("data/blockGroupCentroids.geojson", "r")as infile:
        centroids = json.load(infile)
        centroidsDictionary = {}
        for f in centroids["features"]:
            gid = f["properties"]["AFFGEOID"].replace("1500000US","15000US")
...
def addCentroidsToCsv():
    centroids = getCentroids()
    with open("data/R11583882_SL150.csv","Ur")as infile:
        ...
        count = 0
        with open("data/census_withCentroids.csv","w")as outfile:
